<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between pb-6 border-b border-slate-100">
      <div>
        <h2 class="text-2xl font-bold text-slate-800 flex items-center">
          <Flame class="w-6 h-6 text-primary-500 mr-2" /> 热门游戏
        </h2>
        <p class="text-slate-500 mt-1">这里展示最受欢迎的Galgame</p>
      </div>
    </div>
    ...
    <div v-else-if="games.length === 0" class="flex flex-col items-center justify-center py-20 text-slate-400 bg-white rounded-2xl border border-slate-100 border-dashed">
      ...
      <p class="text-lg font-medium text-slate-500">暂无热门游戏数据</p>
...
import { Flame, Loader2, Gamepad2 } from 'lucide-vue-next';
...
const loadPopularGames = async () => {
    try {
      ...
      error.value = '';
      const response = await apiService.games.getPopularGames();
